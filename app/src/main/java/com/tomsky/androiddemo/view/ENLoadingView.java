...
public class ENLoadingView extends SurfaceView {
    ...
    private AtomicBoolean mCapture = new AtomicBoolean(false);
    ...
    private void draw() {
        ...
        if (mCapture.get()) {
            ...
            for (int i = 0; i < 4 ; i++) {
                drawItem(bitCanvas, mFraction[i], i);
            }
            if (mCaptureCallback != null) {
                mCaptureCallback.loadingCaptured(bitmap);
    ...
    public boolean isShow() {
        return mCurrentState == STATE_SHOW;
    }
    ...
    public interface CaptureCallback {
        void loadingCaptured(Bitmap bitmap);
    }
}